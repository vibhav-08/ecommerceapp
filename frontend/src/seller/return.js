            ...
            <MDBCardBody className="p-4">
                <div className="d-flex flex-row mb-4 pb-2">
                    <div className="flex-fill">
                        ...
                        <p className="text-muted"> Status : {props.status}</p>
                    </div>
                ...
                <MDBContainer className="py-5 h-100">
                    <MDBRow className="justify-content-center h-100">
                        <MDBCol md="10" lg="8" xl="6">
                            {orderedItems.map((order) => <OrderItem status={order.order_status} cart_id={order.id} item_name={order.name} quantity={order.quantity} seller_email={order.seller_email} customer_email={order.customer_email} />)}
                        </MDBCol>
                    </MDBRow>
                </MDBContainer>
            </section>
